    ...
    try {
      ...
      const toolsPayload = (await toolsResponse.json()) as {
        ...
        tools: {
          ...
          ocr: { available: boolean; path: string | null; engine: string | null };
        };
      };
      ...
      expect(toolsPayload.tools.ocr.available).toBe(false);
      expect(toolsPayload.tools.ocr.engine).toBe(null);
