...
describe("YouTube captionTracks extra branches", () => {
  it("prefers bootstrap player captions when embedded watch-page URLs look stale", async () => {
    const html =
      ...
      '{"languageCode":"en","baseUrl":"https://example.com/stale?ip=0.0.0.0"}' +
      "]}}};</script>" +
      "</head><body></body></html>";
    ...
    const fetchMock = vi.fn<[RequestInfo | URL, RequestInit?], Promise<Response>>((input) => {
          ...
          jsonResponse({
            captions: {
              playerCaptionsTracklistRenderer: {
                captionTracks: [{ baseUrl: "https://example.com/fresh", languageCode: "en" }],
              },
            },
          }),
        );
      ...
      if (url.startsWith("https://example.com/fresh") && url.includes("fmt=json3")) {
        return Promise.resolve(
          new Response(JSON.stringify({ events: [{ segs: [{ utf8: "Fresh captions" }] }] }), {
            status: 200,
      ...
      if (url.startsWith("https://example.com/stale")) {
        return Promise.reject(new Error(`Should not fetch stale watch-page caption URL: ${url}`));
      }
    ...
    expect(transcript?.text).toBe("Fresh captions");
