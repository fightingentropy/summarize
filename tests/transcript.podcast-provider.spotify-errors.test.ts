...
const baseOptions = {
  ...
  scrapeWebsite: null as unknown as ((...args: unknown[]) => unknown) | null,
  apifyApiToken: null,
...
describe("podcast transcript provider - Spotify error modes", () => {
  ...
  it("does not require the website scraper when the embed page is blocked but it is not configured", async () => {
    const fetchImpl = vi.fn(async (input: RequestInfo | URL) => {
      const url =
        typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
      if (url === "https://open.spotify.com/embed/episode/abc") {
    ...
    const result = await fetchTranscript(
      { url: "https://open.spotify.com/episode/abc", html: "<html/>", resourceKey: null },
      { ...baseOptions, fetch: fetchImpl as unknown as typeof fetch, scrapeWebsite: null },
    );
  ...
  it("errors when the website scraper fallback returns empty content", async () => {
    const fetchImpl = vi.fn(async (input: RequestInfo | URL) => {
      const url =
        typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
      if (url === "https://open.spotify.com/embed/episode/abc") {
    ...
    const scrapeWebsite = vi.fn(async () => ({ markdown: "", html: "" }));
    ...
    const result = await fetchTranscript(
      { url: "https://open.spotify.com/episode/abc", html: "<html/>", resourceKey: null },
      {
        ...
        scrapeWebsite: scrapeWebsite as unknown as typeof baseOptions.scrapeWebsite,
      },
    );

    expect(result.text).toBeNull();
    expect(result.notes).toContain("website scraper returned empty content");
  });
