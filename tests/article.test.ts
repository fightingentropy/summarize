...
describe("article content extraction", () => {
  ...
  it("extracts a single primary article and drops nested nav chrome", () => {
    const html = `
      <html><body>
        <article>
          <nav><a href="/home">Home</a></nav>
          <header><h1>Title</h1></header>
          <p>This paragraph is long enough to be treated as the main article content.</p>
        </article>
      </body></html>
    `;

    const primaryArticle = extractPrimaryArticleHtml(html);

    expect(primaryArticle).toContain("Title");
    expect(primaryArticle).toContain("main article content");
    expect(primaryArticle).not.toContain("<nav");
    expect(primaryArticle).not.toContain("Home");
  });
});
