...
describe("YouTube transcript provider module", () => {
  ...
  it("returns a precise age-restricted unavailable reason", async () => {
    const result = await fetchTranscript(
      {
        url: "https://www.youtube.com/watch?v=abcdefghijk",
        html: '<script>var ytInitialPlayerResponse = {"playabilityStatus":{"status":"LOGIN_REQUIRED","reason":"Sign in to confirm your age"}};</script>',
        resourceKey: null,
      },
      {
        ...baseOptions,
    ...
    expect(result.notes).toContain("confirm your age");
  });
  ...
  it("reuses captionTracks duration metadata instead of refetching player duration", async () => {
    captions.fetchTranscriptFromCaptionTracks.mockResolvedValue({
      text: "Creator caption",
      segments: null,
      durationSeconds: 2220,
    ...
    expect(captions.fetchYoutubeDurationSecondsViaPlayer).not.toHaveBeenCalled();
  });
