...
import { stripHiddenHtml } from "../src/content/link-preview/content/visibility.js";
...
describe("hidden content stripping", () => {
  ...
  it("handles mixed-case attribute values on hidden candidates", () => {
    const html = `
      <html><body>
        <div aria-hidden="TRUE">Hidden aria uppercase</div>
        <input type="HIDDEN" value="Hidden input uppercase" />
        <p>Visible</p>
      </body></html>
    `;
    ...
    const cleaned = stripHiddenHtml(html);
    expect(cleaned).toContain("Visible");
    expect(cleaned).not.toContain("Hidden aria uppercase");
    expect(cleaned).not.toContain("Hidden input uppercase");
  });
});
