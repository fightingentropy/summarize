...
function isMissingSlidesDependencyError(message: string): boolean {
  ...
  return (
    ...
    lower.includes("slide ocr is unavailable") ||
    lower.includes("missing swiftc") ||
...
export function writeSlidesBackgroundFailureWarning({
  ...
  if (isMissingSlidesDependencyError(message)) {
    ctx.io.stderr.write(
      `${theme.dim("Install ffmpeg + yt-dlp for --slides. macOS uses Vision OCR for --slides-ocr; source/dev builds may need Xcode Command Line Tools.")}\n`,
    );
  }
