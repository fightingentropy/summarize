...
export async function summarizeMediaFile(
  ...
  const envForRun = ctx.envForRun ?? ctx.env;
...
6. Local whisper.cpp (recommended, free):
   ...
   (or set SUMMARIZE_WHISPER_CPP_BINARY)
...
See: https://github.com/openai/whisper for setup details`);
  ...
  const firecrawlScraper = createWebsiteScraperChain({
    fetchImpl: ctx.trackedFetch,
    exaApiKey: envForRun.EXA_API_KEY ?? null,
    cloudflareApiToken: envForRun.CLOUDFLARE_API_TOKEN ?? null,
    cloudflareAccountId: envForRun.CLOUDFLARE_ACCOUNT_ID ?? null,
    firecrawlApiKey: envForRun.FIRECRAWL_API_KEY ?? null,
  });
