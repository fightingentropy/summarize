...
import {
  ensureSummarizeUserFiles,
  loadSummarizeConfig,
  loadSummarizeLocalEnv,
  mergeConfigEnv,
  mergeFallbackEnv,
} from "../config.js";
...
export function prepareRunEnvironment(
  ...
  let envForRun: Record<string, string | undefined> = noColorFlag
    ...
    : { ...inputEnv };
  ensureSummarizeUserFiles({ env: envForRun });
  envForRun = mergeFallbackEnv({
    env: envForRun,
    fallbackEnv: loadSummarizeLocalEnv({ env: envForRun }),
  });
