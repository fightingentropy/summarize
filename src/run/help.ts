...
export function buildProgram() {
  return new Command()
    ...
    .option("--slides-ocr", "Run OCR on extracted slides (uses macOS Vision OCR).", false)
...
${heading("Env Vars")}
  ...
  EXA_API_KEY           optional website extraction fallback (fast text crawl)
  CLOUDFLARE_API_TOKEN  optional website extraction fallback (Browser Rendering markdown)
  CLOUDFLARE_ACCOUNT_ID required with CLOUDFLARE_API_TOKEN
  FIRECRAWL_API_KEY     optional website extraction fallback (last-resort Markdown)
  ...
  SUMMARIZE_VISION_OCR_HELPER optional path to a prebuilt Vision OCR helper for --slides-ocr
  SUMMARIZE_SWIFTC_PATH optional path to swiftc for fallback building of the Vision OCR helper
