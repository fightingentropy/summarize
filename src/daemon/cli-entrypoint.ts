...
export async function resolveCliEntrypointCandidatesFromWindowsShim(
  ...
  const candidates = [
    path.resolve(shimDir, "node_modules", "summarize", "dist", "cli.cjs"),
    path.resolve(shimDir, "node_modules", "summarize", "dist", "cli.js"),
    path.resolve(shimDir, "..", "summarize", "dist", "cli.cjs"),
    path.resolve(shimDir, "..", "summarize", "dist", "cli.js"),
  ];

  try {
    const contents = await fs.readFile(shimPath, "utf8");
    const tokenMatch = contents.match(
      /(?:%~dp0|%dp0%|\$basedir)[^"'\\r\\n]*node_modules[\\/]+summarize[\\/]+dist[\\/]+cli\.(?:cjs|js)/i,
    );
    const matches = Array.from(
      contents.matchAll(
        /["']?([^"'\\r\\n]*node_modules[\\/]+summarize[\\/]+dist[\\/]+cli\.(?:cjs|js))["']?/gi,
      ),
    );
    const preferred =
      tokenMatch?.[0] ??
