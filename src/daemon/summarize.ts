...
function buildStreamMetaForExtracted(
  extracted: ExtractedLinkContent,
): NonNullable<StreamSink["writeMeta"]> extends (data: infer T) => void ? T : never {
  return {
    inputSummary: buildInputSummaryForExtracted(extracted),
  ...
  const ctx = createDaemonUrlFlowContext({
    ...
    hooks: {
      ...
      onExtracted: (content) => {
        ...
        sink.writeMeta?.(buildStreamMetaForExtracted(content));
        writeStatus?.("Summarizing…");
      },
