...
export function parseSlidesConfig(root: Record<string, unknown>, path: string) {
  ...
  const ocrModeRaw = typeof value.ocrMode === "string" ? value.ocrMode.trim().toLowerCase() : "";
  const ocrMode =
    ocrModeRaw === "fast" || ocrModeRaw === "accurate"
      ? (ocrModeRaw as SlidesOcrMode)
      : ocrModeRaw.length > 0
        ? (() => {
            throw new Error(
              `Invalid config file ${path}: "slides.ocrMode" must be "fast" or "accurate".`,
            );
          })()
        : undefined;
  const ocrLanguageCorrection =
    typeof value.ocrLanguageCorrection === "boolean" ? value.ocrLanguageCorrection : undefined;
  const dir =
  ...
  return enabled ||
    typeof ocr === "boolean" ||
    ocrMode ||
    typeof ocrLanguageCorrection === "boolean" ||
    dir ||
    ...
    ? {
        ...
        ...(ocrMode ? { ocrMode } : {}),
        ...(typeof ocrLanguageCorrection === "boolean" ? { ocrLanguageCorrection } : {}),
...
export function parseOutputConfig(root: Record<string, unknown>, path: string) {
  ...
  if (typeof value !== "undefined" && !isRecord(value)) {
    throw new Error(`Invalid config file ${path}: "output" must be an object.`);
  }
  const legacyLanguage =
    typeof root.language === "string" && root.language.trim().length > 0
      ? root.language.trim()
      : undefined;
  const language =
