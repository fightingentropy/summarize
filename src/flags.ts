...
export type WebsiteScrapeMode = "off" | "auto" | "always";
export type FirecrawlMode = WebsiteScrapeMode;
...
export function parseWebsiteScrapeMode(raw: string): WebsiteScrapeMode {
  const normalized = raw.trim().toLowerCase();
  if (normalized === "off" || normalized === "auto" || normalized === "always") return normalized;
  throw new Error(`Unsupported --website-scrape: ${raw}`);
}

export const parseFirecrawlMode = parseWebsiteScrapeMode;
