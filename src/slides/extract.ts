...
import {
  getSlidesOcrAvailability,
  resolveSlidesOcrPath,
...
export async function extractSlidesForSource({
  ...
  return withSlidesLock(
    ...
    async () => {
      ...
      if (settings.ocr && !ocrPath) {
        ocrPath = await resolveSlidesOcrPath(env);
        if (!ocrPath) {
          throw new Error(
            "Slide OCR is unavailable (macOS Vision OCR requires Xcode Command Line Tools, or skip --slides-ocr).",
          );
        }
      }
      const ocrEnabled = Boolean(settings.ocr && ocrPath);
      const ocrAvailable = getSlidesOcrAvailability(env).available;
      ...
      try {
        ...
        if (ocrEnabled && ocrPath) {
          const ocrStartedAt = Date.now();
          logSlides?.(`ocr start count=${renamedSlides.length} mode=parallel workers=${workers}`);
          const ocrOptions = resolveVisionOcrRuntimeOptions({
            recognitionLevel: settings.ocrMode,
            useLanguageCorrection: settings.ocrLanguageCorrection,
          });
