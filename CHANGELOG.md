...
- npm packaging: publish CLI with `pnpm publish` so `summarize-core` is version-pinned in published metadata (no `workspace:*` in registry package).
...
- Media: refactor routing helpers and size policy (#48, thanks @erlinhoxha).
...
- ESM-only: `summarize` + `summarize-core` no longer support CommonJS `require()`; the CLI binary is now ESM.
...
- Core: expose lightweight URL helpers at `summarize-core/content/url` (YouTube/Twitter/podcast/direct-media detection).
...
- Packaging: `summarize-core` now ships a CJS build for `require()` consumers (fixes `pnpm dlx summarize --help` and the published CLI runtime).

## 0.7.0 - 2025-12-26

### Highlights

- Packages: split into `summarize-core` (library) + `summarize` (CLI; depends on core). Versions are lockstep.
...
- `summarize` CLI shipped via `summarize` (plus optional library exports).
