...
- Website: `bun run summarize -- "<url>" --model <model> --length short`
- YouTube: `bun run summarize -- "<url>" --model <model> --youtube auto`
- File URL: `bun run summarize -- "<url>" --model <model>`
- Local file: `bun run summarize -- "<path>" --model <model>`

## Capture

- Log: stdout + stderr, exit code, and timing line.
- Note extraction path (HTML vs Exa/Cloudflare/Firecrawl vs YouTube transcript).
