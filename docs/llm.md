...
- `~/.summarize/.env` (optional): local machine secrets for the CLI; process env and project `.env` still win.
- `~/.summarize/config.json` `env` (optional): fallback non-secret env defaults when higher-precedence env sources are missing/blank.
