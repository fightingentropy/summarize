...
`summarize` supports a JSON config file for defaults plus a local `.env` file for secrets.
...
On first CLI run, `summarize` bootstraps `~/.summarize/config.json` with explicit defaults and backfills missing default keys later. Actual API key values should live in `~/.summarize/.env` or your normal shell env, not in `config.json`.
...
3. `~/.summarize/.env`
4. Config file `env`
5. Legacy config file `apiKeys` (mapped to env names)
...
```json
{
  ...
  "cache": {
    ...
    "maxMb": 512,
    "ttlDays": 30,
    "media": { "enabled": true, "maxMb": 2048, "ttlDays": 7, "verify": "size" }
  },
...
Local secrets file (`~/.summarize/.env`):
...
Use `~/.summarize/.env` for secrets. Use config `env` for non-secret defaults that you still want versioned in the generated config.
...
- Prefer `env` for website scraper credentials because Cloudflare also needs `CLOUDFLARE_ACCOUNT_ID`.
- Legacy `apiKeys.cloudflare` maps only to `CLOUDFLARE_API_TOKEN`; it does not set `CLOUDFLARE_ACCOUNT_ID`.
- External website scraper order is Exa -> Cloudflare -> Firecrawl.
- When the CLI sees real secret values in `config.json`, it migrates them into `~/.summarize/.env` and leaves blank placeholders in config.
...
- `ocrMode`: `fast` or `accurate`
- `ocrLanguageCorrection`: Vision OCR language correction toggle on macOS

## CLI defaults

Configure local CLI backends and let Codex use your own local default model when `model` is blank:
