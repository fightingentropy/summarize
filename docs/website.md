...
- If extraction looks blocked or too thin, it can retry via the external website scraper chain (Markdown).
...
- With `--format md`, the CLI defaults to `--markdown-mode readability` (Readability article HTML as the Markdown input).
  - Use `--website-scrape always` to try external website scrapers first.
...
- `--website-scrape off|auto|always` (`--firecrawl` remains as a deprecated alias)
...
- Optional website scraper providers:
  - `EXA_API_KEY` -> Exa contents API
  - `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` -> Cloudflare Browser Rendering Markdown
  - `FIRECRAWL_API_KEY` -> Firecrawl fallback
- Provider order: Exa -> Cloudflare -> Firecrawl
